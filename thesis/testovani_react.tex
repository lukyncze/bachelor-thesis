...
V Translator komponentě potřebujeme ukládat vstupní hodnotu a výstupní jazyk z vnořených komponent. Dále při každé změně těchto hodnot zavoláme API, k čemuž využijeme \emph{useEffect}.
...
Ve funkci \emph{getAllCountries} využijeme knihovnu \emph{axios} a převzatý requestHandler. 
Balíček \emph{Axios} slouží jako HTTP klient pro tvorbu asynchronních dotazů, zatímco requestHandler umožňuje otypování příchozí odpovědi. Po ošetření chyb aktualizujeme patřičné stavy, které následně z hooku vrátíme. 
...
Opakování asynchronních dotazů při chybě zajistíme pomocí knihovny \emph{axios-retry}. Opakování nakonfigujeme v souboru \emph{main.tsx}. 
...
Jde o uživatelem hádané země s vlajkou a vzdáleností od \emph{randomCountry}. K převodu využijeme JavaScriptové funkce z jiného souboru. 
K vypočtení vzdálenosti použijeme knihovnu \emph{calculate-distance-between-coordinates}, která obsahuje funkci \emph{getDistanceBetweenTwoPoints}. Jednotlivé prvky pole \emph{enrichedGuessedCountries} pak vykreslíme v rámci JSX.
...
Pro routování využijeme knihovnu \emph{react-router-dom}. Začneme vytvořením souboru s cestami (\emph{appRoutes}).
