...
\usepackage{graphicx}
\usepackage{alltt}  % pro zapis programoveho kodu
\usepackage{listings} % pro zapis programoveho kodu
\usepackage{pdfpages}
...
\definecolor{svetleseda}{rgb}{.85,.85,.85}
\definecolor{tmaveseda}{rgb}{.4,.4,.4}
\definecolor{fialova}{rgb}{0.65, 0.12, 0.82}
\usepackage[bookmarkstype={toc},colorlinks,bookmarksnumbered,%
unicode,linkcolor={modra},%
citecolor={zelena},urlcolor={modra}]{hyperref}

% nastaveni pro zobrazeni zdrojoveho kodu
\renewcommand{\lstlistingname}{Zdrojový kód}% Listing -> Algorithm
\renewcommand{\lstlistlistingname}{Seznam zdrojových kódů}% List of Listings -> List of Algorithms
\lstdefinelanguage{TypeScript}{
  ...
  ndkeywordstyle=\color{tmaveseda}\bfseries,
  identifierstyle=\color{black},
  sensitive=false,
  comment=[l]{//},
  morecomment=[s]{/*}{*/},
  commentstyle=\color{fialova}\ttfamily,
...
\lstset{
  language=TypeScript,
  backgroundcolor=\color{svetleseda},
  extendedchars=true,
...
Praktickou částí práce je návrh a implementace webových aplikací, na základě kterých srovnáme implementace ve vybraných SPA frameworcích. 
Práce tak čtenáři nabízí nejen teoretický přehled, ale především praktické zkušenosti s moderními technologiemi frontendu. 
Hlavní přínos práce spočívá v poskytnutí uceleného pohledu na vybrané SPA frameworky, což čtenářům usnadní výběr vhodného nástroje pro budoucí projekty 
a přispívá k hlubšímu porozumění aktuálním trendům ve vývoji frontendu.}
