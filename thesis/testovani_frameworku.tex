...
% \subsubsection{Instalace projektů}

% K vytvoření nového projektu v jednotlivých frameworcích budeme potřebovat NodeJS a npm (případně jiného správce balíčků).
...
% 		Angular   & \begin{tabular}[c]{@{}l@{}}npm init @angular@latest nazev-projektu\\ cd nazev-projektu\\ npm install -D tailwindcss postcss autoprefixer\\ npx tailwindcss init -p\\ manuální konfigurace TailwindCSS (\iadresa{https://tailwindcss.com/docs/guides/angular})\\ npm run start\end{tabular} \\ \hline
% 		React     & \begin{tabular}[c]{@{}l@{}}npm create vite@latest\\ cd nazev-projektu\\ npm install -D tailwindcss postcss autoprefixer\\ npx tailwindcss init -p\\ manuální konfigurace TailwindCSS (\iadresa{https://tailwindcss.com/docs/guides/vite\#react})\\ npm run dev\end{tabular}                \\ \hline
% 		Svelte    & \begin{tabular}[c]{@{}l@{}}npm create vite@latest\\ cd nazev-projektu\\ npx svelte-add@latest tailwindcss (\iadresa{https://github.com/svelte-add/tailwindcss})\\ npm install\\ npm run dev\end{tabular}                                                                                   \\ \hline
% 		\end{tabular}
% 	\label{tab:test}
% \end{table}
